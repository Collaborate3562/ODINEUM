...
function Mint(this: any) {
  ...
  const calculateTimeLeft = () => {
    ...
    const difference = +new Date(`April 21, ${year} 23:00:00`) - +new Date();
  
    let timeLeft = {};
  
    if (difference > 0) {
      timeLeft = {
        days: Math.floor((difference - (1000 * 60 * 60 * 14)) / (1000 * 60 * 60 * 24)),
        hours: Math.floor(((difference - (1000 * 60 * 60 * 14)) / (1000 * 60 * 60)) % 24),
        minutes: Math.floor(((difference - (1000 * 60 * 60 * 14)) / 1000 / 60) % 60),
        seconds: Math.floor(((difference - (1000 * 60 * 60 * 14)) / 1000) % 60)
      };
    }
      ...
      if (order) {
        if(!isPromoter) {
          try {
            ...
            console.log(ethers.utils.parseEther(String(number*price)));

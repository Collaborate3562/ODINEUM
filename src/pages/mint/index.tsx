...
function Mint(this: any) {
  ...
  const [price, setPrice] = useState<number>(0.22);
    ...
    if (active) {
      ...
      if (order) {
        ...
        try {
          ...
          const res = await contract.mint(number, level, {from: account, value: ethers.utils.parseUnits(String(number*price), 18)});
          res
            .wait()
            .then(async (result: any) => {
              ...
              if (events.length > 0) {
                ...
                for (let i = 0; i < number; i += 1) {
                  ...
                  console.log('TokenId',events[i * 2 + 1].args.nftID);
  ...
  const createTrade = async () => {
    ...
    if (active) {
      ...
      // if(approvedAddress !== process.env.REACT_APP_MARKET_ADDRESS) {
      //   const approve = await nftContract.approve(process.env.REACT_APP_MARKET_ADDRESS, selectedNFT.tokenId);
      //   await approve.wait();
      // }
      // const transfer = await nftContract.transferFrom(account, process.env.REACT_APP_MARKET_ADDRESS, selectedNFT.tokenId);
      // await transfer.wait();
      ...
      if (isAuction) {
        ...
        res
          ...
          .catch((err: any) => {
            ...
            console.log("Put On Auction:", err);
          });
      } else {
        console.log("fixed")
        console.log(selectedNFT.tokenId, minPrice, Number(parseUnits(minPrice.toString())), paymentType);
        try {
